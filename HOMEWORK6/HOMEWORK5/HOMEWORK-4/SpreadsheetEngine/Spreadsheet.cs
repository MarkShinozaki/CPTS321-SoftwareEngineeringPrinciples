    ...
    public class Spreadsheet : INotifyPropertyChanged
    {
        ...
        public void EvaluateCell(int rowIndex, int columnIndex, string cellText)
        {
            ...
            else if (cellText[0] == '=')
            {
                ...
                if (expTree.Variables.Keys.Where(k => !this.cellValues.ContainsKey(k)).ToList().Count > 0)
                {
                    this.GetCell(rowIndex, columnIndex).Value = "!(bad reference)";
                    this.cellValues[this.GetCellName(rowIndex, columnIndex)] = 0;
                }
                else if (expTree.Variables.ContainsKey(this.GetCellName(rowIndex, columnIndex)))
                {
                    this.GetCell(rowIndex, columnIndex).Value = "!(self reference)";
                    this.cellValues[this.GetCellName(rowIndex, columnIndex)] = 0;
                }
                else
                {
        ...
        /// <summary>
        /// Loads spreadsheet from an XML file.
        /// </summary>
        /// <param name="stream">Stream of selected XML file.</param>
        public void LoadFromXML(Stream stream)
        {
            this.undoStack.Clear();
            this.redoStack.Clear();

            this.InitializeSpreadsheetCells(this.RowCount, this.ColumnCount);
                    ...
                    switch (reader.NodeType)
                    {
                        case XmlNodeType.Element:
                            if (reader.Name == "cell")
                            {
                                ...
                                this.GetCell(cellName).CellText = reader.GetAttribute("cellText");
                                this.GetCell(cellName).BGColor = Convert.ToUInt32(reader.GetAttribute("bgColor"));
                            }
        ...
        /// <summary>
        /// Saves spreadsheet to an XML file.
        /// </summary>
        /// <param name="stream">Stream of selected XML file.</param>
        public void SaveToXml(Stream stream)
        {
            using (XmlWriter writer = XmlWriter.Create(stream))
            {
                ...
                foreach (var cell in this.spreadsheetCells)
                {
                    if (cell.IsNonDefault())
                    {
                        writer.WriteStartElement("cell");
                        writer.WriteAttributeString("cellName", this.GetCellName(cell.RowIndex, cell.Columnindex));
                        writer.WriteAttributeString("cellText", cell.CellText);
